...
baseline = "simcse"  # or "random"
data = pd.read_json("../semeval-2026-task-4-datasets/semeval-2026-task-4-dev-v1/dev_track_b.jsonl", lines=True)

if baseline == "simcse":
    model = SimCSE(
        "track-b/track-b-simcse/weights"
    )
    embeddings = model.encode(
        data["text"],
        batch_size=8,
        show_progress_bar=True)
if baseline == "storyemb":
    model = SentenceTransformer("track-b/track-b-story-emb/weight")
...
np.save(f"track-b/track-b-story-emb/output/track-b-{baseline}.npy", embeddings)
